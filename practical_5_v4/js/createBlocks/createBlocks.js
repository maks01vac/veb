...
function createAndInsertResultHtmlWithTemplatedData(htmlTemplateContainerId, data, targetHtmlContainerId) {
...
function createNewAndPopularBlocks(dataCourses) {
    ...
    for (let i = 0; i < dataCourses.data.length; i++) {
        ...
        if (dataCourses.data[i].attributes.promotionType == 'NEW') {
            ...
            createAndInsertResultHtmlWithTemplatedData(
                CONST.NEW_AND_POPULAR_COURSES_SAMPLE_CONTAINER, dataNewCourses, CONST.NEW_COURSES_HTML_CONTAINER_SELECTOR);
            ...
            createAndInsertResultHtmlWithTemplatedData(
                CONST.NEW_AND_POPULAR_COURSES_SAMPLE_CONTAINER, dataPopularCourses, CONST.POPULAR_COURSES_HTML_CONTAINER_SELECTOR);
        }
    }
}
